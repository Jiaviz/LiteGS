...
)
{
    ...
    if (degree > 0) {
        for (int c = 0; c < 3; c++)
        {
            dRGB_dx[c] += -SH_C1 * SH_rest[2][c][index]; // from -SH_C1 * x * SH_rest[2] term
            dRGB_dy[c] += -SH_C1 * SH_rest[0][c][index]; // from -SH_C1 * y * SH_rest[0] term
            dRGB_dz[c] += +SH_C1 * SH_rest[1][c][index]; // from +SH_C1 * z * SH_rest[1] term
        }
        ...
        if (degree > 1)
        {
            ...
            for (int c = 0; c < 3; c++)
            {
                dRGB_dx[c] += (SH_C2[0] * y * SH_rest[3][c][index] +           // d(xy)/dx = y
                ...
                dRGB_dy[c] += (SH_C2[0] * x * SH_rest[3][c][index] +           // d(xy)/dy = x
                ...
                dRGB_dz[c] += (SH_C2[1] * y * SH_rest[4][c][index] +          // d(yz)/dz = y
